...
* To manage timeline templates referenced by rules one can set a templates directory using the optional `timeline_template_dir` value (included above). If a timeline-templates directory is explicitly specified in a CLI command, the config value will be ignored.
