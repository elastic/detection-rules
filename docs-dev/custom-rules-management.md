...
* Filter-only rules: KQL rules with an empty `query` (relying solely on `filters`) require `CUSTOM_RULES_DIR` to be set at load and export time to pass validation and serialize correctly. Threat match rules with an empty `threat_query` (relying solely on `threat_filters`) are always valid and exported correctly regardless of `CUSTOM_RULES_DIR`.
