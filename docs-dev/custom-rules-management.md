...
* Filter-only rules: KQL rules with an empty `query` (relying solely on `filters`) and threat match rules with an empty `threat_query` are only supported for custom rules. `CUSTOM_RULES_DIR` must be set at load and export time for these rules to pass validation and serialize correctly.
