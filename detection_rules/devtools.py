...
from .utils import dict_hash, get_etc_path, get_path, github_version_check, load_dump
...
def build_release(
    ...
    err_msg = (
        f'No `registry_data` in package config. Please see the {get_etc_path("package.yaml")} file for an'
...
def get_release_diff(
    pre: str, post: str, remote: Optional[str] = "origin"
...
@click.argument(
    "local-repo", type=click.Path(exists=True, file_okay=False, dir_okay=True), default=get_path("..", "integrations")
)
@click.option(
    "--token",
    required=True,
...
def integrations_pr(
    ...
    assert (
        Path(actual_target_directory).absolute() == Path(target_directory).absolute()
...
def update_navigator_gists(
    directory: Path, token: str, gist_id: str, print_markdown: bool, update_coverage: bool
) -> list:
...
def event_search(
    query, index, language, date_range, count, max_results, verbose=True, elasticsearch_client: Elasticsearch = None
):
...
@click.pass_context
@add_client("elasticsearch")
def rule_event_search(ctx, rule, date_range, count, max_results, verbose, elasticsearch_client: Elasticsearch = None):
    ...
    if schema == "endpoint":
        ...
        if endpoint_target not in optional_endpoint_targets:
            raise click.BadParameter(
                f"""Invalid endpoint schema target: {endpoint_target}
...
def guide_plugin_convert_(
    contents: Optional[str] = None, default: Optional[str] = ""
