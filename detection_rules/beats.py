...
def get_datasets_and_modules(tree: eql.ast.BaseNode | kql.ast.BaseNode) -> tuple[set[str], set[str]]:
    ...
    for node in tree:  # type: ignore[reportUnknownVariableType]
        if (
            ...
            elif node.left == eql.ast.Field("event", ["dataset"]) or node.left == eql.ast.Field("data_stream", ["dataset"]):
            ...
            elif node.expression == eql.ast.Field("event", ["dataset"]) or node.expression == eql.ast.Field("data_stream", ["dataset"]):
