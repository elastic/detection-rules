...
class QueryValidator:
    ...
    def get_required_fields(self, index: str) -> List[dict]:
        ...
        for pk_int in package_integrations:
            ...
            schema, _ = get_integration_schema_fields(integrations_schemas, package, integration,
                                                      current_version, packages_manifest, {}, {})
    ...
    def get_beats_schema(self, index: list, beats_version: str, ecs_version: str) -> (list, dict, dict):
        ...
        if isinstance(self, EQLValidator):
            beat_schema = beats.get_schema_from_eql(self.ast, beat_types, version=beats_version) if beat_types else None
            schema = ecs.get_eql_schema(version=ecs_version, indexes=index, beat_schema=beat_schema)
        else:
