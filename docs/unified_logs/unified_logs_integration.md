...
- Some Unified Log subsystems are suppressed at the OS level and will not emit events even when the integration debug flag is enabled. Those subsystems require an additional host-level `log config` change. See [Apple Events (`com.apple.appleevents`)](com_apple_appleevents.md) for an example.
