...
We collect alerts generated by Detection Rules in order to monitor adoption and improve them. This is done in line with the [Elastic privacy policy](https://www.elastic.co/legal/privacy-statement). We actively strip PII and other sensitive information before ingesting them. The telemetry for a rule can be toggled within it's config using the `telemetry = true` flag. If this is set to false, we will not collect alert telemetry generated by this rule. We do not collect alerts generated by custom Detection Rules. It is best practice when developing a new Detection Rule to set `telemetry = true` for Elastic to monitor it's effectiveness to determine if we need to make changes to the rules configuration. 
