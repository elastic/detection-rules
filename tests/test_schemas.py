...
class TestESQLValidation(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # These cases exercise the AST/semantic validation (KEEP/METADATA checks) that always runs on
        # load, so the schema-validation passes are disabled regardless of other settings.
        patcher = unittest.mock.patch.dict(os.environ, {"DR_ESQL_VALIDATION": "", "DR_REMOTE_ESQL_VALIDATION": ""})
        patcher.start()
