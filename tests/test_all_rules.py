...
PACKAGE_STACK_VERSION = semver.VersionInfo(*current_stack_version().split("."))
...
class TestRuleMetadata(BaseRuleTest):
    ...
    def test_deprecated_rules(self):
        ...
        for rule_id, entry in deprecations.items():
            ...
            if PACKAGE_STACK_VERSION < semver.VersionInfo(*entry['stack_version'].split(".")):
                continue
    ...
    @unittest.skipIf(PACKAGE_STACK_VERSION < semver.VersionInfo.parse("8.3.0"),
        ...
        for rule in self.production_rules:
            ...
            for build_field, field_versions in build_fields.items():
                ...
                if start_ver is not None and current_stack_ver >= start_ver:
                    if min_stack is None or not semver.VersionInfo.parse(min_stack) >= start_ver:
