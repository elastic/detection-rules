...
class TestKQLtoDSL(unittest.TestCase):
    ...
    def test_wildcard_query_string_escaping(self):
        """Wildcard values converted to query_string.query must escape Lucene
        reserved chars while preserving `*` wildcards, mirroring Kibana's
        toQueryStringQuery (see issue #441).
        """
        def qs(field, query):
