...
import kql
from kql.ast import Exists, Field, FieldComparison, FieldRange, Number, String
...
class TestParser:
    ...
    def validate(self, source, tree, *args, **kwargs):
        ...
        actual = kql.parse(source, *args, **kwargs)
        assert actual == tree, f"Expected {tree}, got {actual}"
    ...
    def test_list_equals(self):
        assert kql.parse("a:(1 or 2)", optimize=False) == kql.parse("a:(2 or 1)", optimize=False)

    def test_number_exists(self):
        assert kql.parse("foo:*", schema={"foo": "long"}) == FieldComparison(Field("foo"), Exists())
